...
public class ValidateSideDelegate implements JavaDelegate {
	...
	private enum SideName {
		BREAD("Garlic Bread"), FRIES("Fries"), TEST("test"), WINGS("Wings");
		...
		for (Side side : sideList) {
			...
			if (isValidSideOrder) {
				...
				throw new InvalidItemException(BPMNErrorList.ERROR_ITEM_INVALID,
						"InValid Side Item" + side + " with Business Key: " + execution.getProcessBusinessKey());
			}
		}
		if (newSideList.isEmpty()) {
			log.error(BPMNErrorList.ERROR_EMPTY_LIST + ": sideList is Empty, with Business key: "
					+ execution.getProcessBusinessKey());
			throw new ListEmptyException(BPMNErrorList.ERROR_EMPTY_LIST,
					"sideList is Empty, with Business key: " + execution.getProcessBusinessKey());
		} else {
