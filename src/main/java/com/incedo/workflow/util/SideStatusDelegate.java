...
public class SideStatusDelegate implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		List<EventSubscription> eventSubscriptions = execution.getProcessEngineServices().getRuntimeService()
				.createEventSubscriptionQuery().eventName("SideStatusMessage").eventType("message").list();
		if (eventSubscriptions.isEmpty()) {
			log.error("Message correlation failed from back house. No Process is ready to receive the message. ");
			throw new MessageCorrelationException(BPMNErrorList.ERROR_MESSAGE_NOT_CORRELATE_FROM_BACK_HOUSE,
					"Message correlation failed from back house. No Process is ready to receive the message.");
		} else {
			log.info("event id : " + eventSubscriptions.get(0));
			execution.getProcessEngineServices().getRuntimeService()
					.createMessageCorrelation("SideStatusMessage").processInstanceBusinessKey(bKey)
					.setVariables(sideStatus).correlate();
