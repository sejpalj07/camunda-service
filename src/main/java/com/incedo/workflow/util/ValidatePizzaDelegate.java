...
public class ValidatePizzaDelegate implements JavaDelegate {
	private enum PizzaName {
		VEGGIE("Veggie Pizza"), CHEESE("Cheese Pizza"), MEAT("Meat Pizza"), CHICKEN("Chicken Pizza");
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		for (Pizza pizza : pizzaList) {
			...
			boolean isValidPizzaOrder = Arrays.stream(PizzaName.values()).anyMatch(t -> t.pizza.equals(name));
			if (isValidPizzaOrder) {
				newPizzaList.add(pizza);
			} else {
				log.error(BPMNErrorList.ERROR_ITEM_INVALID + ": InValid Pizza Item: " + pizza + "\n with Business Key: "
						+ execution.getProcessBusinessKey());
				throw new InvalidItemException(BPMNErrorList.ERROR_ITEM_INVALID,
						"InValid Pizza Item" + pizza + " with Business Key: " + execution.getProcessBusinessKey());
			}
		}
		if (newPizzaList.isEmpty()) {
			log.error(BPMNErrorList.ERROR_EMPTY_LIST + ": PizzaList is Empty with Business key: "
					+ execution.getProcessBusinessKey());
			throw new ListEmptyException(BPMNErrorList.ERROR_EMPTY_LIST,
					"PizzaList is Empty, with Business Key" + execution.getProcessBusinessKey());
		} else {
