...
public class ValidateDrinksDelegate implements JavaDelegate {
	...
	private enum DrinksName {
		SODA("Soda"), TEA("Iced Tea"), FANTA("Fanta"), WATER("Water");
		...
		for (String eachDrink : drinksList) {
			...
			if (isValidDrinkOrder) {
				...
				throw new InvalidItemException(BPMNErrorList.ERROR_ITEM_INVALID,
						"InValid Drinks Item" + eachDrink + " with Business Key: " + execution.getProcessBusinessKey());
			}
		}
		if (newDrinksList.isEmpty()) {
			log.error(BPMNErrorList.ERROR_EMPTY_LIST + ": drinksList is Empty, with Business key: "
					+ execution.getProcessBusinessKey());
			throw new ListEmptyException(BPMNErrorList.ERROR_EMPTY_LIST,
					"drinksList is Empty, with Business key: " + execution.getProcessBusinessKey());
		} else {
