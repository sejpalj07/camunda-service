...
public class SidePrepareOrder implements JavaDelegate {
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		List<EventSubscription> eventSubscriptions = execution.getProcessEngineServices().getRuntimeService()
				.createEventSubscriptionQuery().eventName("sideCreationMessage").eventType("message").list();
		if (eventSubscriptions.isEmpty()) {
			...
			throw new MessageCorrelationException(BPMNErrorList.ERROR_MESSAGE_NOT_CORRELATE,
					"No Process is ready to receive the message. ");
		} else {
			execution.getProcessEngineServices().getRuntimeService().createMessageCorrelation("sideCreationMessage")
					.processInstanceBusinessKey(execution.getProcessInstance().getProcessBusinessKey())
					.setVariables(sideName).correlate();
		}
	}
}